...
docker run -dit --name tdj-apache-app -p 8080:80 -v "$PWD"/docs/_site:/usr/local/apache2/htdocs/ httpd:2.4
docker run --rm --link tdj-apache-app:site dcycle/broken-link-checker:3 http://site
